...
class BloodPresssureData : Fragment() {
    ...
    private lateinit var bpDate: EditText
    private lateinit var bp_data: EditText
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        bpDate = view.findViewById<EditText>(R.id.txtDate)
        bp_data = view.findViewById<EditText>(R.id.txtBP)
...
//        BP.difference
        ...
        btnSaveBP.setOnClickListener{
            ...
            Toast.makeText(requireContext(), "${bpDate.text.toString()} ${bp_data.text.toString()}", Toast.LENGTH_SHORT).show()
            dbHelper.insertBloodPressure(bpDate.text.toString(), bp_data.text.toString().toInt(), dbHelper.fetchUserId())
    ...
    private fun displayData(view: View) {
        val bloodPressureData =
            dbHelper.fetchUserId()?.let { dbHelper.getBloodPressure(it) } // Fetch the data
        ...
        if (bloodPressureData != null) {
            for (record in bloodPressureData) {
                ...
                bloodPressureTextView.text = record.bpData.toString()
